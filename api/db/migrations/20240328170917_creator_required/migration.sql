/*
  Warnings:

  - Made the column `userId` on table `GigListing` required. This step will fail if there are existing NULL values in that column.

*/
-- DropForeignKey
ALTER TABLE "GigListing" DROP CONSTRAINT "GigListing_userId_fkey";

-- AlterTable
ALTER TABLE "GigListing" ALTER COLUMN "userId" SET NOT NULL;

-- AddForeignKey
ALTER TABLE "GigListing" ADD CONSTRAINT "GigListing_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
